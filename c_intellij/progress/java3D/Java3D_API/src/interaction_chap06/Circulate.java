 ...
 * @since Java 3D 1.2.1
 */
public class Circulate extends ViewPlatformAWTBehavior {

    int x = 90;
    int y = 0;
    double x_angle = 0.0;
    double y_angle = 0.0;
    double fart_smart_angle = 0.0;
    private Player_Roaming player_roaming = null;
     ...
     */
    public Circulate(Canvas3D c, int flags) {
    ...
    public Player_Roaming getPlayer_roaming() {
        return player_roaming;
    }

    public void setPlayer_roaming(Player_Roaming player_roaming) {
        this.player_roaming = player_roaming;
    }

    private void camera_fix(MouseEvent evt){
        x_angle = x_angle+Math.PI/180;

        Transform3D view_prev_transform = new Transform3D();
        TransformGroup view = getViewingPlatform().getViewPlatformTransform();
        ...
        double but = Math.sin(x_angle);
        double hut = Math.cos(x_angle);
        Point3d centre = new Point3d(v.x/2,v.y,v.z/2);
        //this.getRotationCenter(centre);
        Point3d eye = new Point3d(v.x+but,2,v.z+hut);
    ...
    private void  mouse_farts(MouseEvent evt){
        ...
        getViewingPlatform().getViewPlatformTransform().getTransform(view_transform);
        Vector3d vector = new Vector3d();
        view_transform.get(vector);
        Vector3d v = player_roaming.getPosition();
        System.out.println(-1*dx*rotXMul);
        player_roaming.rotation(-1,dx*rotXMul);
        x = p;
        y = q;

    }
    private void louse_farts(MouseEvent evt) {
        Transform3D view_transform = new Transform3D();
        getViewingPlatform().getViewPlatformTransform().getTransform(view_transform);
        Vector3d vector = new Vector3d();
        view_transform.get(vector);
        Vector3d v = player_roaming.getPosition();
 //       System.out.println("Cam x:"+vector.x+"   y:"+vector.y+"  z:"+vector.z);
        double  angelo = angleOf(v.x,v.z,vector.x,vector.z);
        System.out.println(angelo);
        if(angelo-fart_smart_angle<0){
            player_roaming.rotation(1,Math.toRadians(angelo-fart_smart_angle));
        }
        else if(angelo-fart_smart_angle>0){
            player_roaming.rotation(1,Math.toRadians(angelo-fart_smart_angle));
        }
...
//        this.setRotationCenter(point);
    ...
    public static double angleOf(double x1,double y1, double x2, double y2) {
        ...
        return 90-Math.toDegrees(result);
    }

    private void louse_fart(MouseEvent evt) {
        Transform3D view_transform = new Transform3D();
        getViewingPlatform().getViewPlatformTransform().getTransform(view_transform);
        Vector3d vector = new Vector3d();
        view_transform.get(vector);
