...
public class Surface {
    ...
    private int color = 1; // 1 white, 0 black
        ...
        public TransformGroup form_floor(int flag) {
            ...
            TransformGroup element = new TransformGroup();
            Transform3D transform = new Transform3D();
            element.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
            element.setCapability(TransformGroup.ALLOW_TRANSFORM_READ);
            ...
            while(width<r){
                ...
                while (length<c) {
                    ...
                    while (!result){
                        result = floor_installment(x,z,flag,element);
                    }
                    x = x-1;
                    length++;
                }
        ...
        private boolean floor_installment(int x, int z, int flag, TransformGroup element){
                ...
                if(checker_outter%2==0) {
                    ...
                    for (int length = 0; length < column; length = length + 2) {
                        if (checker_inner % 2 == 0) {
                           boolean result = false;
                           while (!result){
                               result = obstacles_installment(x,z,obstacles);
                           }
                        }
        ...
        private boolean obstacles_installment(int x, int z, TransformGroup obstacles) {
